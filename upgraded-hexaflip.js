...
(function() {
  ...
  prefixProp = function(prop) {
    ...
    if (document.body.style[camelize(prop)] != null) {
  ...
  ref = ['Transform', 'Perspective', 'PerspectiveOrigin'];
  ...
  HexaFlip = (function() {
    function HexaFlip(el1, sets1, options1) {
      ...
      if (!this.sets) {
        ...
        var yearMin = this.yearMin;
        var yearMax = this.yearMax;
        this.sets = {
          year: (function() {
            var i, results;
            results = [];
      ...
      var width = Math.ceil(((this.size + this.margin * 2) * setsLength) - this.margin * 2);
      if (this.sets.year) {
        width = Math.ceil(((this.size + this.margin * 2) * (setsLength + 1)) - this.margin * 2);
      } else {
        width = Math.ceil(((this.size + this.margin * 2) * setsLength) - this.margin * 2);
      }

      this.el.style.width = width + 'px';

      this.el.style[css.perspective] = this.perspective + 'px';
      this.el.style[css.perspectiveorigin] = this.perspectiveOrigin;
    ...
    HexaFlip.prototype._createCube = function(set) {
      ...
      if (set == 'year') {
        cube.el.style.width = cube.holder.style.width = 2 * this.size + 'px';
      ...
      for (k = 0, len1 = faceNames.length; k < len1; k++) {
        ...
        if (side == 'right' && set == 'year') {
          cube[side].style[css.transform] = (rotation + " translate3d(0, 0, " + (this.size * 3 / 2) + "px)") + (this.horizontalFlip ? 'rotateZ(90deg)' : '');
        } else {
    ...
    HexaFlip.prototype._onTouchLeave = function(e, cube) {
      ...
      if (e.relatedTarget && !cube.el.contains(e.relatedTarget)) {
    ...
    HexaFlip.prototype.setValue = function(settings) {
      var cube, index, key, value, arrayOfValues, i;
      for (key in settings) {
        value = settings[key];
        ...
        if (this.sets[key][0] !== undefined && this.sets[key][0].value !== undefined) {
          arrayOfValues = [];
          for (i = 0; i < this.sets[key].length; i++) {
            arrayOfValues.push(this.sets[key][i].value);
          }

          index = arrayOfValues.indexOf(value);
        } else {
          index = this.sets[key].indexOf(value);
        }

        value = this.sets[key][index];
